...
RUN bash "$NVM_DIR/nvm.sh" && sudo nvm install ${NODE_VERSION}
RUN bash "$NVM_DIR/nvm.sh" && sudo nvm use v${NODE_VERSION}
RUN bash "$NVM_DIR/nvm.sh" && sudo nvm alias default v${NODE_VERSION}
